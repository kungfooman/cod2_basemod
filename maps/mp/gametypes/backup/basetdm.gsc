...
main()
{
	...
	level.spawnPlayer = ::spawnPlayer; // 123123231 // export spawnPlayer()
}
...
{
	mod\dispatcher::onPlayerDisconnect(); // 123123231
...
spawnPlayer()
{
	...
	if (!isDefined(self.pers["weapon"]))
		return;
	if (!isDefined(player.changeTeamAllowed))
		player.changeTeamAllowed = 0;
	if (game["state"] == "playing")
	{
		if ( ! player.changeTeamAllowed)
		{
			player iprintln("^8[NOTICE] ^7You are respawning next round.");
			return;
		}
	}
	player.changeTeamAllowed = 0;
