...
lookAtRaw()
{
	...
	//originStart = player getEye() + (0,0,getcvarint("look_offset"));

	//stance = player getFuckingStance();
	stance = "stand";
	if (stance == "stand")
		originStart = player getEye() + (0,0,20); // 20 is perfect!
	else
		originStart = player getEye() + (0,0,getcvarint("offset")); // 20 is perfect!
	//originStart = player getTagOrigin("j_head");
	//originStart = player getTagOrigin("tag_aim");
	angles = player getPlayerAngles();
	forward = anglesToForward(angles);
